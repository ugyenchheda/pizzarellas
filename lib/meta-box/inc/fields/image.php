...
if ( ! class_exists( 'RWMB_Image_Field' ) )
{
	class RWMB_Image_Field extends RWMB_File_Field
	{
		...
		static function kaya_admin_enqueue_scripts()
		{
			...
			parent::kaya_admin_enqueue_scripts();
		...
		static function get_uploaded_images( $images, $field )
		{
			...
			if ( count( $images ) <= 0  )
				$classes[] = 'hidden';
		$ul = '<ul class="%s %s" data-field_id="%s" data-delete_nonce="%s" data-reorder_nonce="%s" data-force_delete="%s" data-max_file_uploads="%s">';
			$html = sprintf(
				$ul,
				implode( ' ', $classes ),
				$field['id'],
			...
			$li = '
				<li id="item_%s">
					...
					<div class="rwmb-image-bar">
						...
						<a title="%s" class="rwmb-delete-file" href="#" data-attachment_id="%s">close</a>
					</div>
				</li>
			';
