...
if ( ! class_exists( 'RW_Meta_Box' ) )
{
	...
	class RW_Meta_Box
	{
		...
		function __construct( $meta_box )
		{
			...
			add_action( 'admin_enqueue_scripts', array( $this, 'kaya_admin_enqueue_scripts' ) );

			// Add additional actions for fields
			$fields = self::kaya_get_fields( $this->fields );
			foreach ( $fields as $field )
			{
				call_user_func( array( self::get_class_name( $field ), 'kaya_add_actions' ) );
			}

			// Add meta box
			add_action( 'add_meta_boxes', array( $this, 'kaya_add_meta_boxes' ) );
		 ...
		 * @return void
		 */
		function kaya_admin_enqueue_scripts()
		{
			...
			foreach ( $fields as $field )
			{
				...
				call_user_func( array( self::get_class_name( $field ), 'kaya_admin_enqueue_scripts' ) );
			}
		 ...
		 * @return array
		 */
		static function kaya_get_fields( $fields )
		{
			$all_fields = array();
			foreach ( $fields as $field )
			{
				...
				if ( isset( $field['fields'] ) )
					$all_fields = array_merge( $all_fields, self::kaya_get_fields( $field['fields'] ) );
			}
