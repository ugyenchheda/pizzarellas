...
if ( !class_exists( 'RWMB_Field ' ) )
{
	class RWMB_Field
	{
		 ...
		 * @return void
		 */
		static function kaya_add_actions() {}
		...
		{
			$kaya_meta = get_post_meta( $post_id, $field['id'], !$field['multiple'] );

			// Use $field['std'] only when the meta box hasn't been saved (i.e. the first time we run)
			$kaya_meta = ( !$saved && '' === $kaya_meta || array() === $kaya_meta ) ? $field['std'] : $kaya_meta;

			// Escape attributes for non-wysiwyg fields
			if ( 'wysiwyg' !== $field['type'] )
				$kaya_meta = is_array( $kaya_meta ) ? array_map( 'esc_attr', $kaya_meta ) : esc_attr( $kaya_meta );

			$kaya_meta = apply_filters( "rwmb_{$field['type']}_meta", $kaya_meta );
			$kaya_meta = apply_filters( "rwmb_{$field['id']}_meta", $kaya_meta );
